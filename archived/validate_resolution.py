#!/usr/bin/env python3
"""
Validate mesh resolution convergence by running the same physical case at
three different resolutions (nx = 7, 13, 26) and comparing temperature at 
the board center vs time. The three curves must visually converge as nx increases.
"""

import numpy as np
import matplotlib.pyplot as plt
from PCB_solver_tr import PCBDomain, HeaterPatch, EdgeDirichletBC, PCB_solver_main
...
def run_convergence_study():
    ...
    # Three resolutions to test: mesh points = [7, 13, 26, 100, 200]
    ...
    for nx in nx_values:
        ...
        X, Y, _, _, _ = domain.discretize(nx, ny)

        # Single heater at center with EXPLICIT fixed physical size (resolution-independent)
        # heater_size = L/10 = 10 mm (consistent across all resolutions)
    ...
    print("Plot saved to 'convergence_study.png'")
    ...
    for nx, T_f in zip(nx_values, T_final_values):
        print(f"  nx={nx:2d}: T_center = {T_f:.6f} K")
    
    # Check convergence: differences should decrease
    diffs = []
    for i in range(len(T_final_values) - 1):
        diff = abs(T_final_values[i+1] - T_final_values[i])
        diffs.append(diff)
        print(f"  |T_center(nx={nx_values[i+1]}) - T_center(nx={nx_values[i]})| = {diff:.2e} K")
    
    # Check if monotonically converging
    is_converging = all(diffs[i] >= diffs[i+1] for i in range(len(diffs)-1))
    convergence_msg = "✓ CONVERGING" if is_converging else "✗ NOT CONVERGING (check mesh)"
    print(f"\nConvergence status: {convergence_msg}")
    print("="*60)
